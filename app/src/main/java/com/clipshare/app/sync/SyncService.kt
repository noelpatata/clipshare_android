...
import com.clipshare.app.certs.CertStore
...
class SyncService : Service() {
    ...
    private fun startDiscovery() {
        ...
        val d = DiscoveryManager(this) { name, host, port, tls, _ ->
    ...
    private fun buildSocket(host: String, port: Int, tls: Boolean, verifyName: Boolean): WsClient? {
        ...
        val hello = Protocol.hello(Prefs.deviceName(this), "android", VERSION)
    ...
    private fun receive(clip: Protocol.Clipboard) {
        ...
        if (image != null) {
            ...
            writeClipboardImage(image, clip.mime ?: "image/png")
            AppState.onReceivedImage(this, clip.mime ?: "image/png", image.size, clip.from)
        } else {
            val text = clip.text
            if (text != null) {
